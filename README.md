...
### Visual Design
- Real-time SVG bowl that updates as you select ingredients
- Animated steam rising from the bowl
- Japanese-style red ceramic bowl with chopsticks
- Japanese wave decorative patterns
- Bilingual labels (English + Japanese characters)
- Noto Serif JP typography for an authentic feel
...
app/
  ...
  page.tsx            # Entry point rendering RamenBuilder
  globals.css         # Tailwind config, design tokens, custom theme

components/
  ramen-builder.tsx   # Main builder orchestrator with step navigation
  ramen-bowl-svg.tsx  # SVG bowl with broth, noodles, toppings, steam
  soup-selector.tsx   # Soup base selection grid
  noodle-selector.tsx # Noodle type selector with custom icons
  topping-selector.tsx# Topping toggle grid
  order-summary.tsx   # Live summary panel with reset/complete actions

lib/
  ramen-data.ts       # All soup, noodle, and topping data + TypeScript types
